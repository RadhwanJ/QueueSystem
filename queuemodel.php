...
{
    abstract public function waitFullBuffer($queueFull);
    abstract public function processing($message);
}

abstract class AbstractConsumerGroup 
{
    abstract public function attachToGroup(AbstractConsumer $consumer);
    abstract public function detachFromGroup(AbstractConsumer $consumer);
    abstract public function autoScaleComsumerGroup(AbstractQueue $queue, $criteria);
    abstract public function partition(AbstractQueue $queue, $criteria);
}

abstract class AbstractQueue 
{   
    abstract public function checkQueueSize($messagesNumber);
...
{
    abstract public function waitEmptyBuffer($queueEmpty);
    abstract public function publish($message, AbstractQueue $queue); 
}
...
class Consumer extends AbstractConsumer
{
     ...
     * @return boolean 
     */
    public function waitFullBuffer($queueFull)
    {
        $result = true;
        if($queueFull == 0) {
            $result = false;
